...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid) {
	    case D_FLIST:
	    {
	        ...
	        pInfo[playerid][pFaction] = ide;
	        
			SendFormMessage(playerid,COLOR_LIGHTBLUE,"You sucessfully joined the %s!",GetFactionNameByID(ide));
		}
...
/*
ocmd:cabs(playerid,params[])
{
	if(pInfo[playerid][pWanteds]>0)return SCM(playerid,COLOR_RED,"You can't call a Taxi with Wanteds!");
	
}*/
ocmd:joinfaction(playerid,params[])
{
	if(pInfo[playerid][LoggedIn] == 0) return SCM(playerid,COLOR_RED,"You are not logged in!");
	if(pInfo[playerid][pFaction]!=0) return SCM(playerid,COLOR_RED,"You are already in a Faction. Use /quitfaction to leave!");
	if(pInfo[playerid][pWanteds]>0)return SCM(playerid,COLOR_RED,"You can't join a Faction with Wanteds!");
	if(IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, COLOR_GREY, "You can't change your Faction in your vehicle!");
	if(pInfo[playerid][dTimer] > 0) return SendClientMessage(playerid, COLOR_GREY, "You are involved in a fight, you can't change your Faction now!");
	ShowPlayerDialog(playerid, D_FLIST, DIALOG_STYLE_LIST, "Faction-List", "LS:PD\nSF:PD\nLV:PD\nFBI\nArmy\nGrove Street\nYakuza\nLa Cosa Nostra\nAtzecas\nVagos\nRifas", "Choose", "Cancel");
	return 1;
}

//ShowPlayerDialog(playerid, D_FLIST, DIALOG_STYLE_LIST, "Faction-List", "LS:PD\nSF:PD\nLV:PD\nFBI\nArmy\nGrove Street\nYakuza\nLa Cosa Nostra\nAtzecas\nVagos\nRifas", "Choose", "Cancel");
stock GetFactionNameByID(ide)
{
	new name[35];
	switch(ide)
	{
...
ocmd:cab(playerid,params[])
{
	...
	new choosed[MAX_PLAYER_NAME],string[128];
    if(sscanf(params, "s[30]", choosed))
    {
		SendClientMessage(playerid, -1, "|__________________ Cab Options  __________________|");
		SendClientMessage(playerid, -1, "Use: /accept [name]");
		SendClientMessage(playerid, -1, "Commands: Duty");
		SendClientMessage(playerid, -1, "|________________________________________________|");
		return 1;
	}
	if(strcmp(choosed,"duty",true) == 0)
	{
	    if(pInfo[playerid][pWanteds]>0)return SCM(playerid,COLOR_RED,"You can't go Duty with Wanteds!");
	}
}*/
